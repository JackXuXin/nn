...
function MJPRoomRecord:showPrivateRoomRecord(flag,msg)
	...
	for _,info in pairs(msg.infos) do
        ...
        -- --包牌
        ui_players[info.seat].ui_pack_card_num:setString(info.bao_pai .. "次")
...
function MJPRoomRecord:init(callback)
    ...
    --标题
    display.newSprite("res/ShYMJ/PrivateRoomRectrd/img_top_bg_njmj.png")
    	:center()
    	:addTo(self)
    ...
    for i=1,4 do
    	...
    	display.newTTFLabel({text = "点炮次数：",size = 20})
    		...
    		:addTo(ui_rectrd_info_bg)
    	--分数字
    	display.newTTFLabel({text = "分数：",size = 25,color = score_color[i]})
    		:pos(96,68)
    		:addTo(ui_rectrd_info_bg)
        --包牌次数字
        display.newTTFLabel({text = "包牌：",size = 25,color = score_color[i]})
            :pos(96,37)
            :addTo(ui_rectrd_info_bg)
		...
		--分数
		ui_players[i].ui_score_num = display.newTTFLabel({text = "140分",size = 25,color = score_color[i]})
			:setAnchorPoint(0,0.5)
    		:pos(133,68)
    		:addTo(ui_rectrd_info_bg)

        --包牌次数
        ui_players[i].ui_pack_card_num = display.newTTFLabel({text = "3次",size = 25})
            :setAnchorPoint(0,0.5)
            :pos(133,37)
            :addTo(ui_rectrd_info_bg)
    end
