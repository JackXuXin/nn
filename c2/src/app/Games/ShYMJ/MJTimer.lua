...
function MJTimer:setTimer(time, direct)
    local pos_x
    local pos_y
    local max = self.callBack:getMaxPlayers()
    ...
    elseif direct == "right" then
        pos_x = display.width - 70
        pos_y = display.cy+78
    elseif direct == "top" then
        --pos_x = 331
       -- if max == 2 then
        pos_x = display.width-260
        --end
        pos_y = display.height - 70
    ...
    self:clearTime()
    self:timeStart(time)
    progTimer:setPosition(cc.p(pos_x,pos_y+15.5))
...
function MJTimer:timeStart(total_time,left_time)
    ...
    label_time:setPosition(cc.p(45,45.5))
    ...
    local move_point = {cc.p(45,85+6),cc.p(90,85+6),cc.p(90,2+8),cc.p(2,2+8),cc.p(2,85+6),cc.p(45,85+6)}
