...
function MJOutCard:ctor()
    ...
    frame = cc.SpriteFrame:createWithTexture(texture, cc.rect(0, 0, 113, 157))
...
function MJOutCard:outCard(card, direct)
    ...
    if showCard == nil and self.callBack then
        showCard = display.newSprite("#ShYMJ/img_GameUI_sendcardframe.png")
        :addTo(self.callBack, 53)
        :setScale(1.2)
        display.newSprite(imgName)
        :pos(58-1, 76+1)
        :addTo(showCard,nil,999)
        :setScale(0.833)
    ...
    local sprite = showCard:getChildByTag(999)
    if sprite ~= nil then
