...
local rightCombin
local pengCombin
local ming_gangCombin
local bu_gangCombin = {}
local an_gangCombin = {}
local flowerCombin = {}
flowerCombin["bu_gang"] = {}
flowerCombin["an_gang"] = {}
local cardCombin = {}
cardCombin["an_gang"] = {}
cardCombin["bu_gang"] = {}
local playerSex = {}
...
function MJPlayers:ctor()
    ...
    ming_gangCombin = display.newSprite("ShYMJ/img_hint_background.png")
    ...
    :pos(0,128)
    :addTo(ming_gangCombin)
    hit_gang:setLocalZOrder(2)

    self:addCombin("ming_gang")
    ming_gangCombin:setTouchEnabled(true)                   
    ming_gangCombin:addNodeEventListener(cc.NODE_TOUCH_EVENT, function (event)
        if event.name == "began" then
            return true
        elseif event.name == "ended" then
            if ming_gangCombin:hitTest(cc.p(event.x, event.y), false) then
                    -- sound.combinCard("gang",playerSex["bottom"],0)
                self:onCombinButton("ming_gang")
            end
        end
    end)
    ...
    --补杠
    for i = 1, 3 do
        bu_gangCombin[i] = display.newSprite("ShYMJ/img_hint_background.png")
        :pos(display.right-91-2*182, display.bottom + 250)
        -- :setScale(182/288)
        :hide()
        :addTo(self)
        ...
        local hit_gang = display.newSprite("ShYMJ/img_hint_background_gang.png")
        :pos(0,128)
        :addTo(bu_gangCombin[i])
        hit_gang:setLocalZOrder(2)

        self:addCombin("bu_gang", i)
        bu_gangCombin[i]:setTouchEnabled(true)                   
        bu_gangCombin[i]:addNodeEventListener(cc.NODE_TOUCH_EVENT, function (event)
            if event.name == "began" then
                return true
            elseif event.name == "ended" then
                if bu_gangCombin[i]:hitTest(cc.p(event.x, event.y), false) then
                    self:onCombinButton("bu_gang", i)
                end
            end
        end)
    end
    --暗杠
    for i = 1, 3 do
        an_gangCombin[i] = display.newSprite("ShYMJ/img_hint_background.png")
        :pos(display.right-91-2*182, display.bottom + 250)
        -- :setScale(182/288)
        :hide()
        :addTo(self)
        ...
        :pos(0,128)
        :addTo(an_gangCombin[i])
        hit_angang:setLocalZOrder(2)

        self:addCombin("an_gang", i)
        an_gangCombin[i]:setTouchEnabled(true)                   
        an_gangCombin[i]:addNodeEventListener(cc.NODE_TOUCH_EVENT, function (event)
            if event.name == "began" then
                return true
            elseif event.name == "ended" then
                if an_gangCombin[i]:hitTest(cc.p(event.x, event.y), false) then
                    self:onCombinButton("an_gang", i)
                end
            end
        end)
    end
...
function MJPlayers:hideAllButton()
    ...
    --     angangCombin[i]:hide()
    -- end
    ming_gangCombin:hide()
    
    for i = 1, #an_gangCombin do
        an_gangCombin[i]:hide()
    end
    for i = 1, #bu_gangCombin do
         bu_gangCombin[i]:hide()
    end
    --要牌器
    AddCardLayer:hide()
    editBox2:hide()
...
function MJPlayers:onCombinButton(combin, index)
    self:hideAllButton()
    if combin == "an_gang" or combin == "bu_gang" then
...
function MJPlayers:setCombin(combin, card, index)
    ...
    elseif combin == "bu_gang" then
     ...
     elseif combin == "an_gang" then
        ...
        local flower = flowerCombin[combin][index]
        flower[1]:setSpriteFrame(cc.SpriteFrameCache:getInstance():getSpriteFrame(string.format("ShYMJ/img_cardvalue_cpg%d.png", card)))
        return an_gangCombin[index]
        
    elseif combin == "hu" then
        return hunButton
    elseif combin == "ignore" then
        return dropButton
    end
end

function MJPlayers:allowCombin(combin)
    ...
    if count > 0 then
        local sprite = {}
        local drop_x = 0
        local check = {"right", "center", "left", "peng", "ming_gang","an_gang","bu_gang", "hu","ignore"}
        for i = 1, #check do
            local index = 0
            for j = 1, count do
                if check[i] == combin[j].mode then
                    index = index + 1
                    if index > 3 then
                        break
                    end
                    sprite[#sprite+1] = self:setCombin(combin[j].mode, combin[j].card, index)
                    -- if check[i] ~= "gang" then
                    --     break
                    -- end
                    if combin[j].mode == "ignore" then
                        --todo
                        drop_x = 90
                    end
                end
