...
function MJWalls:ctor()
    ...
    for i = 1, 9 do
        local frame = cc.SpriteFrame:createWithTexture(texture, cc.rect((i-1)*45, 0, 45, 46))
    ...
    for i = 1, 6 do
        local frame = cc.SpriteFrame:createWithTexture(texture, cc.rect((i-1)*32, 0, 32, 33))
...
function MJWalls:showWalls(show, dice1, dice2, direct)
    if show == true then
        ...
        local animation1 = cc.Animation:createWithSpriteFrames(frames1, 1/20)
        local animate1 = cc.Animate:create(animation1);
        rotateDice1:runAction(cc.Sequence:create(animate1))

        local animation2 = cc.Animation:createWithSpriteFrames(frames2, 1/20)
        local animate2 = cc.Animate:create(animation2);
        rotateDice2:runAction(cc.Sequence:create(animate2))

        local dis = math.random(5)*10
        local divide =  math.random(3) + 1
        if direct == "left" then
