...
void  YVConfigManager::setConfig(ConfigType type, ...)
{
	...
	case ConfigIsOverSea:
	{
		//在android上用bool型会挂，但是改成int就正常了
		this->isOverSea = va_arg(ap, int);
		break;
	}
	...
	case ConfigGroupCacheNum:
	{
        this->groupCacheNum = va_arg(ap, uint32);
	    break;
	}

	case ConfigGroupHistoryChatNum:
	{
		this->groupHistoryChatNum = va_arg(ap, uint32);
		break;
	}
...
YVConfigManager::YVConfigManager()
{
	...
	groupHistoryChatNum = 5;
}
uint32 YVConfigManager::getGroupCacheNum()
{
	return groupCacheNum;
}