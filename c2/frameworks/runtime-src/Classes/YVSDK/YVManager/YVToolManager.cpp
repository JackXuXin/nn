...
bool YVToolManager::setRecordTime(uint32 timeNum, int volume)
{
...
bool YVToolManager::speechVoice(YVFilePathPtr path)
{
	...
	if (path->getLocalPath().empty() && path->getUrlPath().empty())
	{
		return false;
	}
	if (path->getState() != BothExistState && path->getState() != OnlyLocalState)
	{
		return false;
	}
