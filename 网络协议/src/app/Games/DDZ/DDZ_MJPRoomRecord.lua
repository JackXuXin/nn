...
local DDZ_MJPRoomRecord = class("DDZ_MJPRoomRecord",function()
...
function DDZ_MJPRoomRecord:ctor()
end

function DDZ_MJPRoomRecord:showPrivateRoomRecord(flag,msg,weixinImage,majiang)
    ...
    recordBg:setTexture("ShYMJ/PrivateRoomRectrd/img_top_bg_ddz.png")
    -- end

	--房间号
	if ui_room_id then
		ui_room_id:setString("房间号:" .. msg.roomid)
	end
	...
	if ui_room_type then
        ...
        -- end
        ui_room_type:setString(msg.all .. "局")
	end

	for _,info in pairs(msg.players) do
		--头像
		...
		if #msg.players == 2 then
			if info.seat == 1 then
				ui_players[info.seat].ui_node:pos(240,0)
			elseif info.seat == 2 then
				ui_players[info.seat].ui_node:pos(380,0)
			end
		elseif #msg.players == 3 then
            if info.seat == 1 then
                ui_players[info.seat].ui_node:pos(100,0)
            elseif info.seat == 2 then
                ui_players[info.seat].ui_node:pos(150,0)
            elseif info.seat == 3 then
                ui_players[info.seat].ui_node:pos(200,0)
            end
        else
...
function DDZ_MJPRoomRecord:init(callback)
...
function DDZ_MJPRoomRecord:clear()
	ui_players = {}
	ui_room_type = nil
	ui_room_id = nil
    recordBg = nil
