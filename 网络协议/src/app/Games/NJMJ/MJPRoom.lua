local NJMJPRoom = class("NJMJPRoom",function()
...
local table_session
local players = {}
local roomEnd = nil  --解散按钮

function NJMJPRoom:ctor()

end

function NJMJPRoom:showSeat(params)
    ...
    roomEnd = cc.ui.UIPushButton.new({ normal = "res/Image/PrivateRoom/btn_dissolution_01.png", pressed = "res/Image/PrivateRoom/btn_dissolution_01.png" })
...
function NJMJPRoom:tableSession( tableSession )
   table_session = tableSession 
end

function NJMJPRoom:setPlayer(seat, name, score, sex, viptype,creator,imageid)
    ...
    local ready = nil
     ...
     --背景
    player_bg = display.newSprite("res/Image/PrivateRoom/img_height_touxiangBG.png")
        :addTo(self)
    ...
    --设置微信头像
    util.setHeadImage(head:getParent(), imageid, head, image, seat)
...
function NJMJPRoom:clearPlayer(seat)
...
function NJMJPRoom:showRoomEndState(flag)
    if roomEnd then
        roomEnd:setVisible(flag)
    end
end

function NJMJPRoom:ready(seat)
end

function NJMJPRoom:init(callback)
    self.callback = callback
    return self
end

function NJMJPRoom:clear()
    room_session = nil
    players = {}
    roomEnd = nil
end
