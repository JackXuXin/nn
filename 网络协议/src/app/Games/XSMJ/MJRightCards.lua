...
function MJRightCards:addOut(card)
    ...
    -- else
        ...
        :pos(display.right - 245 + 62 * (math.floor(count/11) % 2), display.cy + 10 -11*38/2 + (count%11)*38-14*math.floor(count/22))
        :addTo(cardLevel)
...
function MJRightCards:arrangeOut()
    ...
    if count  > 0 then
        ...
        for i = number, count do
            ...
            sprite:pos(display.right - 245+62*(math.floor((count-1)/11)%2), display.cy + 10 -11*38/2 + (count-i+1)*38+12*math.floor((count-1)/22))
            flower:setSpriteFrame(cc.SpriteFrameCache:getInstance():getSpriteFrame(string.format("ShYMJ/img_cardvaluePaichiZY%d.png", card)))
...
function MJRightCards:removeCombin(combin, card)
    ...
    for i = 1, count do
        ...
        sp:pos(display.right - 85, display.bottom + 200 + (i-1)*91)
    end
end
...
function MJRightCards:addCombin(combin, card, direct)
    ...
    :pos(display.right - 85, display.bottom + 200 + count*96)
...
function MJRightCards:arrangeCards(mode)
    ...
    if count > 0 then
        for pos = 1, count do
            ...
            if mode == "center" then
                sprite:pos(display.right - 90 , display.cy + 80 +  27*count/2 -pos*27)
            elseif mode == "right" then
                sprite:pos(display.right - 90 , display.cy + 80 + 27*5 -(pos-1)*27-5)
            elseif mode == "link" then
                sprite:pos(display.right - 90 , display.cy + 80 + 27*6 -(pos-1)*27-5)
